...
void main()
{
	...
	color += texture2D(scene, vec2(gl_TexCoord[0].s,gl_TexCoord[0].t - pixelSize)).rgb * (gaussianBlur[1].x/norm);
	color += texture2D(scene, vec2(gl_TexCoord[0].s,gl_TexCoord[0].t              )).rgb * (gaussianBlur[1].y/norm);
	color += texture2D(scene, vec2(gl_TexCoord[0].s,gl_TexCoord[0].t + pixelSize)).rgb * (gaussianBlur[1].z/norm);
